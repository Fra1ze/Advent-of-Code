...
gSym = re.compile(r'([-&$*@+=#%/])')
gNum = re.compile(r'(\d{1,})')
rows = schematic.split('\n')
partDict = {r: [] for r in range(len(rows))}
sumPartDict = {r: [] for r in range(len(rows))}
partSum = 0
...
def checkAdj(i, span):
    x = [i-1,i,i+1]
    y = [x for x in range(span[0], span[1])]
    adj = False
...
def doubleGearCheck(s, obj, row):
    c = [x.span() for x in obj['c']]
    keys = list(obj.keys())
    sub = 0
    pT = []
    cT = []
    ...
    if 'p' in keys and 'n' in keys:
        p = [x.span() for x in obj['p']]
        n = [x.span() for x in obj['n']]
    ...
    if row == 0:
        for x in pT:
            if x not in partDict[row-1]:
                partDict[row-1].append(x)
            else:
                sub += int(rows[row-1][i[0]:i[1]])

        for x in cT:
            if x not in partDict[row]:
                partDict[row].append(x)
            else:
                sub += int(rows[row-1][i[0]:i[1]])

    elif row > 0 and row <= len(rows)-1:
        for x in pT:
            if x not in partDict[row-1]:
                partDict[row-1].append(x)
            else:
    ...
    if len(result) == 2:
        ...
        print(result)
        for i in pT:
            z = z * int(rows[row-1][i[0]:i[1]])
        
        for i in cT:
            z = z * int(rows[row][i[0]:i[1]])
...
for r in range(len(rows)):
    symbols = [m.start() for m in gSym.finditer(rows[r])]
    
    if(len(symbols) > 0):
        for s in symbols:
            print(f'{r}:{s} - {rows[r][s]}')
            print(partSum)
            symbol = rows[r][s]
            if(r == 0):
                cur,next = rows[r],rows[r+1]
                cNums = gNum.finditer(cur)
                nNums = gNum.finditer(next)

                if symbol == '*':
                    
                    check = {'c': cNums, 'n': nNums}
                    partSum += doubleGearCheck(s ,check, r)[0]
                else:
                    if(cNums):
                        for m in cNums:
                            if checkSides(s, m.span()):
                                if m.span() not in partDict[r]:
                                    partDict[r].append(m.span())
                                    partSum += int(rows[r][m.span()[0]:m.span()[1]])

                    if(nNums):
                        for m in nNums:
                            if checkAdj(s, m.span()):
                                if m.span() not in partDict[r+1]:
                                    partDict[r+1].append(m.span())
                                    partSum += int(rows[r+1][m.span()[0]:m.span()[1]])

            elif(r > 0 and r+1 <= len(rows)-1):
                prev,cur,next = rows[r-1],rows[r],rows[r+1]
                pNums = gNum.finditer(prev)
                cNums = gNum.finditer(cur)
                nNums = gNum.finditer(next)

                if symbol == '*':
                    check = {'p': pNums,'c': cNums,'n': nNums}
                    partSum += doubleGearCheck(s ,check, r)[0]
                    partSum -= doubleGearCheck(s ,check, r)[1]
                else:
                    if(pNums):
                        for m in pNums:
                            if checkAdj(s, m.span()):
...
#for key in partDict:
#    partSum += sum(partDict[key])

#z = [partDict[x] for x in partDict]
#a = [sum(s) for s in z]

for k in partDict:
    for i in range(len(partDict[k])):
        num = partDict[k][i]
        partDict[k][i] = int(rows[k][num[0]:num[1]])
        
    
    #for i in partDict[k]:
    #    partSum += multiplyList(partDict[k])
